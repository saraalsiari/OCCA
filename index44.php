...
<!-- login end-->
	...
	{    
			    ...
			    else if($line_i[$i] == '"')   // for detecting double quotes
				{
					...
					$operandCount++;
					else
						{
							...
							array_push($operands,substr($line_i,$startIndex,(($endIndex+1)-$startIndex)+1));
						}
				}
			    else
				{
		...
		if(strpos($line_i,"\"") !== false)
		{
			$startIndex = strpos($line_i,"\"");
			$endIndex = strrpos($line_i,"\"");
			$line_i = substr($line_i,0, $startIndex+1) + substr($line_i,$endIndex);
		}
			...
			while(strpos($line_i,$variable) !== false)
				{
					...
					$line_i = substr($line_i,0,$index+1) . substr($line_i,$index+strlen($variable), strlen($variable)-($index+strlen($variable))+1);
