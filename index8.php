...
<html>
 ...
 <head>
		...
		<?php ini_set('memory_limit','128M');// to handle the fetal error regarding memory allocated size
/* 
if (!empty($_POST)) setcookie ("last_post", implode($_POST));
if (!empty($_POST) and $_COOKIE['last_post'] == implode($_POST)) $_POST = []; */
/* if (isset($_POST['code']))
{
	if ($_POST['computeComplexity'] != "empty") //empty can be anything
	{
	$_POST = "empty";
	}
} */
...
<!-- login end-->
        ...
        <div class="center">
            ...
            <form  id= "myForm" method="post">
                <div class="container">
                    <textarea id="code" name="code" placeholder="Enter your code here..." rows="20" cols="50">
					<?php  if(isset($_POST['code']))
					 ...
					 <input class="button_clear" type ="submit" id="computeComplexity" name="computeComplexity" value="Compute Complexity" 
				>
...
$n = $n1+$n2;
$V = $N * log($n) / log(2);
$D = ($n1/2)*($N2/($n2 ?: 1));//This ? to avoid the division by zero error
$L = 1/($D ?: 1);//This ? to avoid the division by zero error
$E = $V*$D;
$T = $E/18;
$B = (pow($E, (2/3))/3000);
...
function extractConstants($line)
{
	...
	if(isset($_POST['computeComplexity']))
	{
		linesNum();	
	}
  ...
  <!-- The `multiple` attribute lets users select multiple files. -->
            ...
            <input class="button" type="button" value="Clear!" onclick="myFunction()">
            <br><br>
